...
// iScanner.next() - это запрос следующего значения

// int range = (max - min) + 1; - запись запроса рандома
// for (int i = 0; i < ((int) Math.random() * range + min); i++) {
// output.add((int) Math.random() * range + min);
//  array[i] = (int) (Math. random () *10 -5 ); - рандом выдаёт дабл в диапазоне от 0.1 до 1, по этому умножаем на 10,
//  а "-5" это диапазон от -5 до 5 не включительно
